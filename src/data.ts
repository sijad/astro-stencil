...
function createProxy<T>(path: string[], renderer: Renderer): PathNode<T> {
  return new Proxy(
    {},
    {
      get(_, prop: string | symbol) {
        ...
        if (prop === "iteration") {
          return () => {
            ...
            return [
              start,
              createProxy(varName ? [varName] : [], renderer),
              createProxy(indexVar ? [indexVar] : [], renderer),
              end,
            ];
          };
        }
        ...
        if (prop === "map") {
          return (callBack: (item: PathNode<T>, i: PathTerminal) => string) => {
            const [start, varName, indexVar, end] = renderer.getIter(path);
        ...
        return createProxy([...path, prop.toString()], renderer);
      },
    },
  ) as PathNode<T>;
}

export function getData<T>(renderer: Renderer): PathNode<T> {
  return createProxy<T>([], renderer);
}
